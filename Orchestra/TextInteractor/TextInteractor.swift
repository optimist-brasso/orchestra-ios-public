...
func <->(property: UITextView, variable: TextModel) -> [AnyCancellable] {
    let propertyToVariable = property.textPublisher.receive(on: RunLoop.main).assign(to: \.value, on: variable)
    let  variableToProperty  = variable.$value.receive(on: RunLoop.main).assign(to: \.text, on: property)
    return [propertyToVariable, variableToProperty]
}
...
public enum Pattern: String, PatternIdentifiable {
    ...
    /// The password type
    case password =   #"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,15}$"# //#"^([a-zA-Z0-9@*#]{8,15})$"# //#"(?=^.{8,}$)(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\s)[0-9a-zA-Z!@#$%^&*()-^*]*$"#
...
class TextInteractor: TextFieldInteractable {
    ...
    private func set(value: String, result: Bool, extra: String = "") -> AppError? {
        if value.isEmpty {
            let error = type.error(isEmpty: true)///LocalizedKey.required(type.name).value
            return AppError.custom(error)
        } else if !extra.isEmpty {
            return AppError.custom(extra)
        }
        else if !result {
            let error  =  type.error(isEmpty: false) // LocalizedKey.validRequired(type.name).value
            return AppError.custom(error)
        }
        return nil
    }
