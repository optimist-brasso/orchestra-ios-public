...
extension PlaylistWireframe: PlaylistWireframeInput {
    
    var storyboardName: String {return "Playlist"}
    
    func getMainView() -> UIViewController {
        let service = PlaylistService()
        let interactor = PlaylistInteractor(service: service)
        let presenter = PlaylistPresenter()
        let screen = PlaylistScreen()
        let viewController = PlaylistViewController(baseScreen: screen)
        viewController.tabBarItem = UITabBarItem(title: LocalizedKey.playlist.value, image: GlobalConstants.Image.TabBar.music, selectedImage: nil)
